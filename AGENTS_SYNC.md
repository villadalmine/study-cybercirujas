...
1. **A queryable code graph exists and is committed.**
   ...
   `get_neighbors`, `shortest_path`). A deterministic Markdown wiki derived
   from the graph lives in `graphify-out/wiki/` (69 articles, `index.md` is
   the agent entry point) — also free, regenerated by `make graph`; its 59
   community names were the only paid step (one claude-cli call).
...
6. **STATUS.md now ends with a "Spend (measured)" footer** (completions,
   output tokens, session windows, per-stage counts), regenerated by every
...
that does not parse, in content committed on or before 2026-08-08:
`lpi-050-100` (4.2, 5.3), `lpi-devops` (5.1), `lpic-3-303` (6.1),
`lpic-3-305` (352.1, 353.4), `lpic-3-306` (364.4), `pca` (1.4, 2.1) — en
and es of each. Predates everything above (first full `make verify` in a
while; the pre-commit hook only inspects staged content, so these walked
in unchecked). Needs a content repair pass: LPI ones are Antigravity's
family, `pca` is CNCF/claude. Free to diagnose: the script prints file,
line and parse error.
