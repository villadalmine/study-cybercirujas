...
Study platform for IT certifications. All content is **AI-generated** from
official sources (syllabi scraped from lpi.org, PDFs from
...
Every piece of study material on this platform — content, exercises, labs,
video scripts and their translations — is **generated by large language
models** (Anthropic Claude: `claude-opus-5`, `claude-opus-4-8`,
`claude-fable-5`; Google Gemini for part of the corpus), under human
direction and mechanical verification, from each certification's official
syllabus. No topic is human-authored.

How that is accounted for, rather than just admitted:

- **Per-topic provenance.** Every `certs/<cert>/<topic>/<lang>/` carries a
  `meta.yaml` naming the backend, the exact model that answered, the
  generation date and the official sources used.
  `scripts/check_provenance.py` enforces it; [MODELS.md](MODELS.md)
  aggregates it.
- **Verified structure, honest limits.** Material passes a quality floor
  before it ever reaches disk, and mechanical audits check citations,
  embedded manifests and deprecated APIs. What no check yet proves is that
  every *explanation* is true — that gap is documented in
  [docs/AUDITOR_DESIGN.md](docs/AUDITOR_DESIGN.md). Treat this material as a
  study aid; the cited official documentation is the authority.
- **EU AI Act, Article 50.** The AI Act's transparency obligations for
  AI-generated content
  ([Article 50](https://artificialintelligenceact.eu/article/50/), with the
  [Commission's guidance](https://digital-strategy.ec.europa.eu/en/faqs/transparency-obligations-under-article-50-ai-act))
  apply since 2 August 2026. This notice plus the per-topic provenance are
  this project's disclosure layer as a deployer. The machine-readable
  marking layer belongs to the model provider — and since August 2026
  Anthropic [embeds invisible watermarks](https://techcrunch.com/2026/08/11/anthropic-says-it-will-watermark-text-generated-by-its-ai-models/)
  (SynthID-Text based) in Claude text output and C2PA metadata in files,
  precisely to satisfy Article 50(2) — so the material here is also
