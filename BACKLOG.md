...
7. **Surface the AI-generated disclosure in the web UI** (EU AI Act Art. 50,
   applies since 2026-08-02). The README now discloses it to repo readers,
   but the students read the SITE — the disclosure belongs at the point of
   use. Cheap and mostly wiring: each topic page already has `meta.yaml`
   (model + date + sources) served alongside the content, so a per-topic
   "AI-generated by <model> on <date>, from <official source>" line plus one
   site-level notice covers the deployer duty. Model-side machine-readable
   marking is already handled upstream (Anthropic watermarks Claude text
   since 2026-08; C2PA for files). Owner decides placement/wording.
