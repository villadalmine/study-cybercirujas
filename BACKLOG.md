...
2. **Translate `cks` and `kcna` out of Spanish-only.** `cks` → English is **in progress and stalled at 6/26** (Domain 1 complete + 2.1) — the run hit the monthly API spend limit at topic 2.2. Resume with `teach cert generate cks --lang en --backend claude`, which skips what already exists; the 20 outstanding topics are listed by `fix_corrupted_content.py`. Then `kcna`, then the other 5 languages for all four CNCF certs. Budget ~7 min per topic, measured.
