---
cert: aws-clf
exam: CLF-C02
snapshot_date: '2026-09-03'
sources:
- https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
topics:
- id: '1.1'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Define the benefits of the AWS Cloud
  topic: 1 - Cloud Concepts
  weight: 6.0
- id: '1.2'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify design principles of the AWS Cloud
  topic: 1 - Cloud Concepts
  weight: 6.0
- id: '1.3'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Understand the benefits of and strategies for migration to the AWS Cloud
  topic: 1 - Cloud Concepts
  weight: 6.0
- id: '1.4'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Understand concepts of cloud economics
  topic: 1 - Cloud Concepts
  weight: 6.0
- id: '2.1'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Understand the AWS shared responsibility model
  topic: 2 - Security and Compliance
  weight: 7.5
- id: '2.2'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Understand AWS Cloud security, governance, and compliance concepts
  topic: 2 - Security and Compliance
  weight: 7.5
- id: '2.3'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS access management capabilities
  topic: 2 - Security and Compliance
  weight: 7.5
- id: '2.4'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify components and resources for security
  topic: 2 - Security and Compliance
  weight: 7.5
- id: '3.1'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Define methods of deploying and operating in the AWS Cloud
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.2'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Define the AWS global infrastructure
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.3'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS compute services
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.4'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS database services
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.5'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS network services
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.6'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS storage services
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.7'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS artificial intelligence and machine learning (AI/ML) services
    and analytics services
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '3.8'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify services from other in-scope AWS service categories
  topic: 3 - Cloud Technology and Services
  weight: 4.25
- id: '4.1'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Compare AWS pricing models
  topic: 4 - Billing, Pricing, and Support
  weight: 4.0
- id: '4.2'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Understand resources for billing, budget, and cost management
  topic: 4 - Billing, Pricing, and Support
  weight: 4.0
- id: '4.3'
  sources:
  - https://d1.awsstatic.com/training-and-certification/docs-cloud-practitioner/AWS-Certified-Cloud-Practitioner_Exam-Guide.pdf
  status: generated
  title: Identify AWS technical resources and AWS Support options
  topic: 4 - Billing, Pricing, and Support
  weight: 4.0
version: '1.0'
weights_published: 4
---

# AWS Certified Cloud Practitioner (CLF-C02)

Snapshot del temario. Completar `topics` con id, title, weight, status y sources
(ver certs/lpi-010-160.md como referencia) y correr `teach cert generate aws-clf`.
