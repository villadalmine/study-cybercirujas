...
def _stats(entries: list[dict]) -> dict:
    """The same numbers main() prints, as a dict — shared by the MODELS.md renderer."""
    n = len(entries)
    kb = statistics.median(e["bytes"] for e in entries) / 1024
    measured = [e for e in entries if e["tokens"]]
    ktok = statistics.median(e["tokens"] for e in measured) / 1000 if measured else 0
    return {
        "n": n, "kb": kb, "ktok": ktok,
        "eff": (kb / ktok) if ktok else 0,
...
def _md_table(per_model: dict[str, list[dict]]) -> list[str]:
    lines = ["| model | n | KB | ktok | KB/1k | floor | code | refs |",
             "|---|---|---|---|---|---|---|---|"]
    for model, entries in sorted(per_model.items(), key=lambda kv: -len(kv[1])):
        s = _stats(entries)
        thin = " ⚠️" if s["n"] < 10 else ""
        lines.append(f"| `{model}`{thin} | {s['n']} | {s['kb']:.1f} | {s['ktok']:.1f} "
                     f"| {s['eff']:.2f} | {s['floor']:.0f}% | {s['code']:.0f} | {s['refs']:.0f} |")
    return lines


def render_markdown() -> str:
    """MODELS.md: the model-comparison dashboard, derived and deterministic.

    Same tree, same bytes — no timestamps, no LLM, nothing invented. Written so
    the owner reads the answer to "which model delivers better material" without
    asking anyone: per-cert tables appear wherever two or more RESOLVED models
    coexist inside one certification, because that is the only fair comparison
    (cross-cert numbers confound model with subject; that confound inverted a
    verdict once — docs/BACKEND_COMPARISON.md).
    """
    rows = observations(None)
    lines = [
        "# Model Comparison",
        "",
        "Generated by `scripts/model_comparison.py --write` — do not edit. "
        "Regenerated with STATUS.md (`make cert`, `make publish`, the timer "
        "pass). Attribution comes from each topic's `meta.yaml`; tokens/cost "
        "from `usage.jsonl`; in-flight topics excluded via the claim system.",
        "",
        "⚠️ = thin evidence (fewer than 10 topics). None of these columns says "
    ...
    for cert in sorted(per_cert):
        ...
        for row in per_cert[cert]:
            if "(unresolved)" not in row["model"] and row["model"] != "unknown":
                models[row["model"]].append(row)
        if len(models) < 2:
            continue
        any_cert = True
        lines += ["", f"### {cert}", ""] + _md_table(models)
    if not any_cert:
        lines += ["", "No certification holds two resolved models yet."]

    per_model: dict[str, list[dict]] = defaultdict(list)
    for row in rows:
        per_model[row["model"]].append(row)
    lines += ["", "## Whole corpus, per model (context, not a verdict)", ""]
    lines += _md_table(per_model)

    per_window = quota_facts.tokens_per_window()
    if per_window:
        lines += ["", f"## What a quota window buys ({per_window:,} output "
                      "tokens/window, measured on this machine)", "",
        ...
        for model, entries in sorted(per_model.items(), key=lambda kv: -len(kv[1])):
            ...
            lines.append(f"| `{model}` | {topics:.1f} | {topics * s['kb']:.0f} |")
    return "\n".join(lines) + "\n"


def write(path: Path | None = None) -> bool:
    """Write MODELS.md; True if it changed. Importable, like status_matrix.refresh."""
    target = path or (REPO / "MODELS.md")
