...
| 3 · **MCP half BUILT 2026-09-14** | the contract existed once phase 2 needed it | `teach/mcp_server.py` serves `list_certs`, `get_syllabus`, `get_topic`, `search_topics` over stdio (`make mcp-setup`, then `.mcp.json`). The multi-pass half still waits for demand for career-level work — several paid calls per question have to buy something a single pass cannot |
